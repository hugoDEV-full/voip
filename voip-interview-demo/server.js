...
function startSimulatedCall({ from = '1000', to = '1001', scenario = 'normal', durationMs = 12000 }) {
...
app.post('/call', async (req, res) => {
  ...
  const call = startSimulatedCall({ from, to, scenario, durationMs });
...
// Automatically generate realistic traffic so the dashboard stays alive like a real system
const AUTO_TRAFFIC = String(process.env.AUTO_TRAFFIC || '').toLowerCase() !== 'false';
if (AUTO_TRAFFIC) {
  const scenarios = ['normal', 'one_way_audio', 'nat_wrong'];
  setInterval(() => {
    try {
      if (state.calls.size >= 3) return;

      const scenario = scenarios[Math.floor(Math.random() * scenarios.length)];
      const from = String(1000 + Math.floor(Math.random() * 50));
      const to = String(2000 + Math.floor(Math.random() * 50));
      const durationMs = 10000 + Math.floor(Math.random() * 8000);

      startSimulatedCall({ from, to, scenario, durationMs });
    } catch (e) {
      console.error('❌ AUTO_TRAFFIC failed:', e);
    }
  }, 15000);
}
