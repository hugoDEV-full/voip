...
if (socket) {
  ...
  socket.on('call_event', (ev) => {
    ...
    if (ev.type === 'RTP_METRIC') {
      const existing = state.calls.get(ev.callId) || { id: ev.callId };
      const latency = Number(ev.latencyMs ?? existing.latency ?? 0);
      const expected = Number(ev.expected ?? 0);
      const received = Number(ev.received ?? 0);
      const packetLoss = expected > 0 ? Math.max(0, Math.min(100, ((expected - received) / expected) * 100)) : 0;

      // Jitter/MOS are approximations for a demo UI.
      const jitter = Math.max(0, Math.round((Math.random() * 4) + (packetLoss > 5 ? 8 : 1)));
      const mos = Math.max(1, Math.min(4.5, Number((4.5 - (packetLoss / 25) - (latency / 400)).toFixed(2))));

      state.calls.set(ev.callId, {
        ...existing,
        latency,
        jitter,
...
async function refreshStats() {
  try {
    ...
    // Some dashboards don't have a raw JSON stats pane (#stats). Update the actual UI widgets instead.
    if (elStats) {
    ...
    const cpuPercent = Math.max(0, Math.min(100, Number(data?.cpu?.percent ?? 0)));
    if (cpuUsage) cpuUsage.textContent = cpuPercent.toFixed(0) + '%';
    if (cpuBar) cpuBar.style.width = cpuPercent + '%';

    const heapUsed = Number(data?.memory?.heapUsed ?? 0);
    const heapTotal = Number(data?.memory?.heapTotal ?? 0);
    const memPercent = heapTotal > 0 ? Math.max(0, Math.min(100, (heapUsed / heapTotal) * 100)) : 0;
    if (memUsage) memUsage.textContent = memPercent.toFixed(0) + '%';
    if (memBar) memBar.style.width = memPercent + '%';

    if (uptime) {
      const uptimeSec = Number(data?.uptimeSec ?? 0);
...
async function refreshCalls() {
  try {
    ...
    const data = await res.json();
    const calls = Array.isArray(data?.activeCalls) ? data.activeCalls : [];

    state.calls.clear();
    for (const c of calls) {
      if (c && c.id) state.calls.set(c.id, c);
    }
    renderCalls();
    updateCounters();
  } catch {
